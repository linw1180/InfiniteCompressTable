# -*- coding: utf-8 -*-
import logging

from ..api import extra_server_api, server_system_api
from ...modCommon import ModName, ModServerSystemName, ModClientSystemName

_server_mod_system = {}

logger = logging.getLogger('{}.Server'.format(ModName))


def get_server(name_space=ModName, system_name=ModServerSystemName):
    """
    获取服务端系统，全局一个单例。

    :return: ClientSystem 返回具体系统的实例，如果获取不到则返回 None
    """
    server = name_space + system_name
    if server not in _server_mod_system:
        _server_mod_system[server] = extra_server_api.get_system(name_space, system_name)
    return _server_mod_system[server]


def listen_engine_events(engine_event_info):
    """
    监听服务端引擎事件

    :param engine_event_info: list[event_name, instance, function] 服务端引擎事件
        event_name: str 事件名
        instance: instance 回调函数所属的类的实例
        func: function 回调函数
    :return:
    """
    for event_name, instance, function in engine_event_info:
        server_system_api.listen_for_event(extra_server_api.get_engine_namespace(),
                                           extra_server_api.get_engine_system_name(),
                                           event_name, instance, function)


def un_listen_engine_events(engine_event_info):
    """
    反注册监听服务端引擎事件

    :param engine_event_info: list[event_name, instance, function] 服务端引擎事件
        event_name: str 事件名
        instance: instance 回调函数所属的类的实例
        func: function 回调函数
    :return:
    """
    for event_name, instance, function in engine_event_info:
        server_system_api.un_listen_for_event(extra_server_api.get_engine_namespace(),
                                              extra_server_api.get_engine_system_name(),
                                              event_name, instance, function)


def listen_client_events(client_event_info):
    """
    监听客户端自定义事件

    :param client_event_info: list[event_name, instance, function] 客户端自定义事件
        event_name: str 事件名
        instance: instance 回调函数所属的类的实例
        func: function 回调函数
    :return:
    """
    for event_name, instance, function in client_event_info:
        server_system_api.listen_for_event(ModName, ModClientSystemName, event_name, instance, function)


def un_listen_client_events(client_event_info):
    """
    反注册监听客户端自定义事件

    :param client_event_info: list[event_name, instance, function] 客户端自定义事件
        event_name: str 事件名
        instance: instance 回调函数所属的类的实例
        func: function 回调函数
    :return:
    """
    for event_name, instance, function in client_event_info:
        server_system_api.un_listen_for_event(ModName, ModClientSystemName, event_name, instance, function)

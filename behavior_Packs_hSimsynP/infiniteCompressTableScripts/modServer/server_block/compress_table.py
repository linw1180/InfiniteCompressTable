...
class CompressTable(Block):
	...
	def on_custom_container_item_swap(cls, args):
		...
		if from_slot == 'output_slot' and isinstance(to_slot, int):
			...
			if take_percent < 1:  # 不允许分堆取出
				return False
			if take_percent == 1 and not to_item:
				print '---------------- 222 -------------------- from_item =', from_item
				# # 背包指定槽位无数据，需要手动进行生成
				# temp_item = copy.deepcopy(from_item)
				# temp_item['count'] = 1
				# spawn_item_to_player_inv(temp_item, player_id, to_slot)
				# print '---------------- 333 -------------------- temp_item =', temp_item
				# item_dict = get_player_item(player_id, ItemPosType.INVENTORY, to_slot)
				# print '---------------- 444 -------------------- item_dict =', item_dict
				...
				# 设置物品自定义tips和标识符
				cls.set_item_custom_tips(player_id, to_slot, temp_item["count"])
				from_item = get_player_item(player_id, ItemPosType.INVENTORY, to_slot)
				print '7777777777777777777777 from_item =', from_item
	...
	def set_item_custom_tips(player_id, slot, compress_count):
		...
		tips = "test已压缩数量 " + "§b§o" + str(compress_count) + "§r"
		...
		# after_change_item = get_player_item(player_id, ItemPosType.INVENTORY, slot)
		# print '999 after_change_item =', after_change_item
		# return after_change_item

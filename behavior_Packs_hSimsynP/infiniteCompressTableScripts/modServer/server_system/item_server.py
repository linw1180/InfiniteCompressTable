...
def on_set_item_custom_tips(args):
	"""
	设置物品自定义tips（根据物品中的extraId数据）
	"""
	print '----------- on_set_item_custom_tips ------------- args =', args
	player_id = args['player_id']
	click_slot = args['slot']
	click_item_dict = get_player_item(player_id, ItemPosType.INVENTORY, click_slot)
	if click_item_dict and click_item_dict['extraId']:
		str_extra_id = click_item_dict['extraId']
		# str ==> dict对象
		# extra_id_dict = ast.literal_eval(str_extra_id)
		extra_id_dict = json.loads(str_extra_id)

		compress_count = extra_id_dict['extraId']['compress_count']

		tips = "已压缩 de 数量 " + "§b§o" + compress_count + "§r"

		# 修改玩家物品的自定义tips和自定义标识符
		print '1111111111111 ret =', change_player_item_tips_and_extra_id(player_id, ItemPosType.INVENTORY, click_slot, tips, str_extra_id)
